    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-green-500 to-green-700 flex items-center justify-center shadow-lg shadow-green-500/25">
            <Tag className="h-7 w-7 text-white" />
          </div>
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-[hsl(var(--foreground))]">{brandName}</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link to={`/prices/new?brandId=${brandId}`}>
            <Button className="bg-gradient-to-l from-green-500 to-green-700 text-white rounded-xl gap-2 shadow-lg shadow-green-500/25">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {data.generic && (
            <div className="flex items-center gap-3 p-3 rounded-xl bg-blue-50 border border-blue-100">
              ...
              <div>
                ...
                <Link to={`/generics/${data.generic.id}`} className="text-sm font-medium text-blue-600 hover:underline">
        ...
        <div className="bg-white rounded-2xl border border-[hsl(var(--border))] shadow-sm p-6">
          <h2 className="font-semibold text-[hsl(var(--foreground))] pb-3 border-b border-[hsl(var(--border))] mb-4 flex items-center gap-2">
            ...
            سجل الأسعار
            {prices.length > 0 && (
              <span className="mr-auto bg-green-100 text-green-700 text-xs font-bold px-2.5 py-0.5 rounded-full">{prices.length}</span>
            )}
          </h2>

          {/* Current price highlight */}
          {currentPrice && (
            <div className="flex items-center justify-between p-4 mb-3 bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl text-white">
              <div>
                <p className="text-white/80 text-xs">السعر الحالي</p>
                <p className="text-2xl font-bold">{formatIQD(currentPrice.unit_price)}</p>
                <p className="text-white/70 text-xs mt-0.5">منذ {formatDate(currentPrice.effective_date)}</p>
              </div>
          ...
          {prices.length > 1 ? (
            <div className="space-y-2 max-h-56 overflow-y-auto">
              {prices.map((price: any) => (
                <div key={price.price_id || price.id} className="flex items-center justify-between p-3 rounded-xl bg-[hsl(var(--muted))]/40">
                  <div>
                  ...
                  {price.is_active && (
                    <span className="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full font-medium">ساري</span>
                  )}
                </div>
              ))}
            </div>
